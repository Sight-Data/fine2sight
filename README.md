...
### 内容自适应(`wrap_mode` / `auto_width`)

帆软的「页面设置>根据单元格内容自动调整」有三档(否/行高/**列宽**),**默认「行高」**,
单元格「其他」可逐格覆盖。转换器 1.1.1 及以前把所有单元格写死成不换行(＝「否」那一档),
内容超长在帆软里会撑高行、转过来却被截断 —— 从 1.2.0 起默认改为换行撑高,与帆软默认档一致。

| 开关 | 默认 | 说明 |
| --- | --- | --- |
| `wrap_mode` / `--wrap-mode` | `row` | `row`=换行撑高行;`none`=截断(1.1.1 及以前的行为) |
| `auto_width` / `--auto-width` | 关 | 给所有列标 `widthMode="auto"`(按内容定宽,需 Sight Report **2.0.25+**) |
| `auto_width_max` / `--auto-width-max` | 无 | 配合上一项:每列宽度上限(pt);不给则由引擎按设计宽度推导 |

> ⚠️ `auto_width` 是**整表一刀切**,不是按帆软的单元格设置驱动。原因:那个设置在 `.cpt` XML
> 里的元素名/属性名/取值编码没有公开文档,手头也没有样例可比对,**猜错会静默产出错版面**。
> 转换时若在模板里发现疑似属性(名字含 `adjust`),会原样列进转换报告的「🔍 自适应取证」一节 ——
> 把那一节反馈给维护者,就能把这两个开关升级成按单元格设置逐格驱动。
